...
### 小改动直接走 main（例外）
- **改动很小且低风险**（bug 修复 / 局部逻辑 / 文案 / 测试向量，通常 ≤ 几个文件、diff 小、无 schema / 基础设施 / 迁移变更）时，**无需开分支或 PR，直接提交到 `main`**。
- 前提仍需满足：`bash scripts/ci.sh` 全绿（lint + 测试），提交信息符合规范。
- 不确定"算不算小"时，默认开分支走 PR。涉及 ❌ 禁止事项里的项（迁移、密钥、基础设施、CI 配置）永远不适用本例外。
...
- ❌ 直接在 main 分支提交大改动 / 高风险改动（这些必须用 PR；小而低风险的改动见「小改动直接走 main」例外）
- ❌ 提交 `.env` 或密钥文件
- ❌ 不运行测试就 push
- ❌ 删除用户数据相关的表（逻辑删除）
