...
// 发送键右侧 "+" 展开的宫格面板（微信样式）：圆角方块图标 + 下方文字。
// 关键点：面板不是遮挡输入框的全屏浮层，而是内联在输入框「下方」的一块区域，
// 展开时输入框同步上移、始终可见（参考微信 + 号面板）。语音聊天（开关弹窗）/
// 语音通话（全屏页）/ 转账（转账输入页）/ 重新开始（回到开场，二次确认）。
export function ChatPlusMenu({ open, isDark, onVoiceChat, onVoiceCall, onTransfer, onRestart }: ChatPlusMenuProps) {
  ...
  return (
    <div
      className="relative z-20 overflow-hidden transition-[max-height,opacity] duration-200 ease-out"
      style={{
        maxHeight: open ? 220 : 0,
        opacity: open ? 1 : 0,
        paddingBottom: open ? 'var(--safe-bottom)' : 0,
      }}
      aria-hidden={!open}
    >
      <div
        className={`mx-3 mb-3 rounded-[24px] px-4 pt-5 pb-4 backdrop-blur-[24px] border ${
          isDark
            ? 'bg-[rgba(26,26,46,0.7)] border-[rgba(255,255,255,0.08)]'
            : 'bg-[var(--color-glass-75)] border-[var(--color-border-glass)]'
        }`}
      >
        <div className="grid grid-cols-4 gap-y-5">
          <button className="flex flex-col items-center" onClick={onVoiceChat}>
            <span className={tile}>
          ...
          <button className="flex flex-col items-center" onClick={onVoiceCall}>
            <span className={tile}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <button className="flex flex-col items-center" onClick={onTransfer}>
            <span className={tile}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <button className="flex flex-col items-center" onClick={onRestart}>
            <span className={tile}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
