      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-[100px]">
          {loading ? (
            ...
            <>
              {/* Hero Card */}
              <div className="relative w-full h-[280px] rounded-[24px] overflow-hidden shadow-[var(--shadow-hero)] mb-4">
                <img
                  src="/assets/backgrounds/background_character_hero.webp"
                  alt=""
                  className="absolute inset-0 w-full h-full object-cover"
                />
                {/* Glass overlay */}
                <div className="absolute inset-0 bg-[rgba(255,255,255,0.35)]" />
                {/* Info */}
                <div className="absolute bottom-[56px] left-0 right-0 text-center">
                  <p className="text-[24px] font-semibold text-[var(--color-ink)]">
                    神无月凛
                  </p>
                  <p className="text-[13px] text-[var(--color-text-secondary)] mt-[6px]">
                    刚刚和你聊过 · 心情：温柔
                  </p>
                </div>
                {/* CTA */}
                <button
                  ...
                  className="absolute right-4 bottom-4 bg-[rgba(255,255,255,0.85)] backdrop-blur-[12px] rounded-[16px] px-4 py-2 text-[15px] font-medium text-[var(--color-cta-text)] active:scale-[0.97] transition-transform shadow-[var(--shadow-soft)]"
                >
                  开始聊天 →
                </button>
              </div>
              ...
              <div className="flex gap-3 mb-5">
                {[
                  ...
                  <button
                    ...
                    className="flex-1 flex flex-col items-center gap-2 py-3 bg-[var(--color-glass-55)] backdrop-blur-[12px] rounded-[20px] border border-[rgba(255,183,197,0.20)] shadow-[var(--shadow-soft)] active:scale-[0.96] transition-transform"
                  >
                ...
                <div className="bg-[var(--color-glass-35)] backdrop-blur-[12px] rounded-[20px] border border-[var(--color-border-glass)] overflow-hidden">
                  {conversations.map((conv) => (
                    <button
                      key={conv.id}
                      onClick={() => navigate('/chat')}
                      className="w-full flex items-center gap-[14px] py-3 px-4 active:bg-[rgba(255,183,197,0.12)] transition-colors"
                    >
                      <Avatar src={conv.avatar} size={56} />
                      <div className="flex-1 min-w-0 text-left">
                        <p className="text-[16px] font-semibold text-[var(--color-ink)]">{conv.name}</p>
                        <p className="text-[14px] text-[var(--color-text-secondary)] truncate">{conv.preview1}</p>
                      </div>
