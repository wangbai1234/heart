...
#   - Node.js 18+ 已安装（前端构建）
#   - backend/.env 文件已创建（从 .env.example 复制并填写；注意 Fish 用 fishaudio.org 网关）
...
# .env 位于仓库根（pydantic-settings 从 CWD 读取；后端进程在 backend/ 下启动，
# 因此下面加载 .env 时用仓库根路径，并把 DATABASE_URL/REDIS_URL 注入进程环境）。
...
# 运行 Alembic 迁移：多 head DAG，必须用 `upgrade heads`（复数）升到所有 head。
# `upgrade head`（单数）会因多 head 报 "Multiple head revisions"。
# 用 heads 而非硬编码版本号，避免"新加迁移忘了改脚本"导致 DB 落后代码。
...
if (-not (Test-Path $EnvFile)) {
    ...
    Write-Yellow "请先执行: Copy-Item .env.example .env  然后填写 API 密钥"
    exit 1
}
...
# ──────────────────────────────────────────────────────────────────────────────
# 2. 加载 .env → 进程环境（供本地 alembic / uvicorn 使用 localhost 连接串）
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
if ($Migrate) {
    Write-Green "运行 Alembic 迁移（upgrade heads）..."
    Invoke-Migrations
    Write-Green "✅ 迁移完成"
    exit 0
}
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. 启动后端 uvicorn（带全部 workers），新开 PowerShell 窗口保持日志可见
# ──────────────────────────────────────────────────────────────────────────────
...
Write-Host ""
Write-Blue "爱发电 Webhook（公网需经 Cloudflare Tunnel 暴露到 yuoyuo.app）："
Write-Host "  https://yuoyuo.app/api/webhooks/afdian?token=<AFDIAN_WEBHOOK_TOKEN>"
Write-Host ""
Write-Blue "常用操作："
