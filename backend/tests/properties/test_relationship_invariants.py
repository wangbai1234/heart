...
def test_inv_r_1_stage_monotonic(before, after):
    ...
    if after_ord < before_ord:
        assert not result, (
            f"INV-R-1: stage regressed from {before['current_stage']} to {after['current_stage']}"
        )
    else:
        assert result, (
            f"INV-R-1: false positive for {before['current_stage']} → {after['current_stage']}"
        )
...
def test_inv_r_4_trust_asymmetry(before, delta):
    ...
    if actual_delta > 0.05:
        assert not result, f"INV-R-4: trust increased by {actual_delta:.3f} (> 0.05)"
    elif actual_delta < -0.20:
        assert not result, f"INV-R-4: trust dropped by {actual_delta:.3f} (< -0.20)"
    else:
        # actual_delta in [-0.20, 0.05] — should pass
        assert result, f"INV-R-4: false positive for trust delta {actual_delta:.3f}"
...
def test_inv_r_6_cold_war_no_progress(state):
    ...
    has_cold_war = any(
        s.get("state_type") == "COLD_WAR" for s in state.get("active_special_states", [])
    )
