...
class TestPromptBuilderDeterministic:
    ...
    def test_same_input_same_output(self, builder, sample_window, sample_l3, sample_hints):
        run_id = uuid4()
...
class TestPromptBuilderSections:
    ...
    def test_window_section_present(self, builder, sample_window, sample_l3, sample_hints):
    ...
    def test_l3_snapshot_section_present(self, builder, sample_window, sample_l3, sample_hints):
        out = builder.build(sample_window, sample_l3, sample_hints, uuid4())
        assert "L3 snapshot" in out
        assert "fact_id=" in out
        assert "self.name" in out

    def test_l3_snapshot_empty(self, builder, sample_window, sample_hints):
        out = builder.build(sample_window, [], sample_hints, uuid4())
        assert "(empty)" in out

    def test_hints_section_present(self, builder, sample_window, sample_l3, sample_hints):
        out = builder.build(sample_window, sample_l3, sample_hints, uuid4())
        assert "Regex hints" in out
        assert "T12" in out
        assert "她叫妙妙" in out

    def test_hints_empty(self, builder, sample_window, sample_l3):
...
class TestPromptBuilderMetadata:
    ...
    def test_metadata_present(self, builder, sample_window, sample_l3, sample_hints):
        run_id = uuid4()
    ...
    def test_prompt_version_and_schema_version(self, builder):
        assert PROMPT_VERSION == "1.0.3"
        assert SCHEMA_VERSION == "1.0.0"

    def test_custom_versions(self, sample_window, sample_l3, sample_hints):
...
class TestPromptBuilderFewShot:
    ...
    def test_few_shot_included(self, builder, sample_window, sample_l3, sample_hints):
        out = builder.build(sample_window, sample_l3, sample_hints, uuid4())
    ...
    def test_system_rules_included(self, builder, sample_window, sample_l3, sample_hints):
        out = builder.build(sample_window, sample_l3, sample_hints, uuid4())
        assert "R1" in out
        assert "R2" in out
        assert "R3" in out
