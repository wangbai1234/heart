...
class LayerCompressor:
    ...
    def _compress_memory(self, layer: PromptLayer, target: int) -> PromptLayer:
        ...
        l4_pattern = re.compile(
            r"\[L4_IDENTITY\](.*?)\[/L4_IDENTITY\]", re.DOTALL
        )
...
def allocate(layers: list[PromptLayer], total_budget: int) -> AllocatedLayers:
    ...
    **Performance target**: < 10 ms per §3.3 Step 4.
    ...
    compressible.sort(key=lambda L: L.priority)

    # -- 3. Calculate non-compressible token cost ----------------------

    nc_tokens = sum(counter.estimate(L.content) for L in non_compressible)
    ...
    total_compressible_est = sum(
        counter.estimate(L.content) for L in compressible
    )

    if nc_tokens + total_compressible_est <= total_budget:
        # No compression needed
        all_allocations = _build_noop_allocations(
            non_compressible + compressible, counter
        )
        return AllocatedLayers(
    ...
    total_allocated = sum(
        counter.estimate(L.content) for L in result_layers
    )
