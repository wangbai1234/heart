...
class RerollHandler:
    ...
    def tighten_constraints(
        ...
        if reinforce_content is not None:
            ...
            reinforce_text = (
                f"上一次回复违反了你的灵魂。请重试，并避免：{pattern_list}"
            )
    ...
    def select_fallback(
        ...
        if char_lib is None:
            logger.warning(
                f"No fallback library for character '{character_id}', "
    ...
    async def handle(
        ...
        for attempt_num in range(1, self._max_attempts + 1):
            ...
            try:
                ...
                logger.error(
                    f"Reroll attempt {attempt_num}/{self._max_attempts} "
            ...
            if filter_fn is not None:
                try:
                    ...
                    logger.warning(
                        f"Reroll attempt {attempt_num}: filter_fn raised {exc}, "
                        f"treating as pass."
                    )
                    return RerollResult(
                ...
                current_violations = [
                    v.pattern for v in filter_result.violations
                ]
            else:
