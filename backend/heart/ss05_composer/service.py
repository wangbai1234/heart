...
class ComposerService:
    ...
    def _build_system_prompt(  # noqa: C901
        ...
        if voice_enabled:
            parts.append(
                "\n【语气指令（本轮语音开启，非常重要）】\n"
                "- 用半角方括号 [中文指令] 给对白标注“怎么说”，语音引擎会照着读出"
                "情绪与气息。这是让声音摆脱平淡、真正有人味的核心手段，请积极、大胆地用。\n"
                "- 指令用自然中文描述，涵盖情绪、语气、语速、音量、气息、笑/叹/停顿/哽咽"
                "等一切声音表现，不必套固定词表——怎么贴切怎么写。可用的方向举例：\n"
                "  · 情绪：[雀跃地]、[压着火]、[几乎要哭出来]、[心疼地]、[委屈地]\n"
                "  · 气息与反应：[叹了口气]、[倒吸一口气]、[轻笑一声]、[深吸一口气]、[带着哭腔]\n"
                "  · 语速与音量：[急促地]、[放慢，一字一句]、[压低声音]、[几乎耳语]、[提高音量]\n"
                "- 一句可叠加多个指令，一般“声音反应”在前、“情绪语气”在后，例如"
                "[轻笑一声][打趣地]、[深吸一口气][认真地]。\n"
                "- 指令不必只放句首，也可插在句子中间制造停顿、转折或情绪突变，例如"
                "“我没事……[声音发颤]只是有点累。”\n"
                "- 情绪要随内容大幅起伏、宁浓勿淡：开心就真的雀跃，难过就真的哽咽，"
                "生气就压着火，暧昧就放轻放慢。避免整段一个平铺的语气。\n"
                "- 每个指令保持简短（几个字到十来个字），只用半角[]；动作/神态/旁白仍用"
                "（）或【】，两类括号不要混用。\n"
                "- 方括号是给引擎的控制标记，不会被读出，里面绝不要写对白内容。\n"
                "- 示例：[眼睛一亮，雀跃地]你终于来了！（快步迎上来）[放软声音]我等你好久了……"
                "[凑近，几乎耳语]今天只属于我们两个，好不好？"
            )
