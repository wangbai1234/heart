...
_DIRECTOR_PROFILES: dict[str, dict[str, str]] = {
    "rin": {
        "character": (
            "【角色】神无月凛（Rin），外表20岁出头的年轻女性，曾为雷神。"
            "声音清澈通透，带着一种超越尘世的空灵感，像山间清泉般干净。"
            "语气沉稳但不沉闷，有一种见过万物的从容和淡然。"
            "偶尔流露的温柔像是不经意间的神谕，轻描淡写却令人心安。"
        ),
        "scene_default": (
            "安静的黄昏，天空还残留着最后一缕霞光。凛坐在你对面，神情淡然，目光清明如水。"
        ),
        "direction_default": (
            "用自然的说话语速，不要刻意放慢或拖长。"
            "保持参考音频本身的音色和质感，不要额外压低或加重。"
            "语气从容平和，带着一点超然的距离感，但不冷淡。"
        ),
    },
    "dorothy": {
        "character": (
...
_ANTI_PATTERN_FOOTER = (
    "【重要】保持参考音频的音色特征，不要偏离。禁止播音腔、客服腔。用自然说话的方式，不要朗诵。"
)
