...
class RelationshipService:
    ...
    def _get_signal_aggregator(self, user_id: UUID, character_id: str) -> "SignalAggregator":
    ...
    async def process_turn(
        ...
        if days_since_last <= 1:
            ...
            days_continuous = int((now - state.first_meeting_at.replace(tzinfo=timezone.utc)).total_seconds() / 86400)
        else:
            days_continuous = 0
