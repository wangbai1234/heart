...
class StagePhaseEngine:
        ...
        if current_stage == RelationshipStage.ACQUAINTANCE:
            ...
            if days_since_last > 60:
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_absence", "reason": f"Absence {days_since_last:.0f}d > 60d"},
                )
            ...
            if state.trust_score < 0.30 and self._sustained_for_days(state, "low_trust", 30):
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_trust", "reason": "Trust < 0.30 sustained"},
                )
            ...
            if days_since_last > 30:
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_absence", "reason": f"Absence {days_since_last:.0f}d > 30d"},
                )
            if state.conflict_debt > 0.6 and self._sustained_for_days(state, "high_conflict", 14):
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_conflict", "reason": "Conflict debt > 0.6 sustained"},
                )
            ...
            if days_since_last > 21:
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_absence", "reason": f"Absence {days_since_last:.0f}d > 21d"},
                )
            ...
            if state.trust_score < 0.50 and self._sustained_for_days(state, "low_trust", 30):
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_trust", "reason": "Trust < 0.50 sustained"},
                )
            if state.conflict_debt > 0.7:
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_conflict", "reason": "Conflict debt > 0.7"},
                )
            ...
            if ritual_broken and state.trust_score < 0.70:
                return StageDecision(
                    ...
                    gate_block_reason={"gate": "regression_ritual", "reason": "Ritual broken + low trust"},
                )
    ...
    def _check_entry_requirements(
        ...
        if target_stage == RelationshipStage.ACQUAINTANCE:
            ...
            if state.total_interactions < eff_interactions:
                return False, f"Need {eff_interactions:.0f}+ interactions, have {state.total_interactions}", {
            ...
            if self._calculate_days_since_first(state) < eff_days:
                return False, f"Need {eff_days:.1f}+ days since first meeting", {
        ...
        # ── ACQUAINTANCE → FRIEND ──
            ...
            if state.total_meaningful_disclosures < eff_disclosures:
                return False, f"Need {eff_disclosures:.0f}+ disclosures, have {state.total_meaningful_disclosures}", {
            ...
            if disclosure_sessions < 2:
                return False, f"Need 2+ distinct disclosure sessions, have {disclosure_sessions}", {
        ...
        # ── FRIEND → CONFIDANT (tuned v1.1) ──
            ...
            if state.trust_score < 0.60:
                return False, f"Need trust >= 0.60, have {state.trust_score:.2f}", {
            ...
            if vuln_sessions < 2:
                return False, f"Need 2+ distinct vulnerability sessions, have {vuln_sessions}", {
            ...
            if state.current_stage == RelationshipStage.FRIEND.value:
                ...
                if days_in_friend < eff_days:
                    return False, f"Need {eff_days:.0f}+ days in FRIEND, have {days_in_friend:.1f}", {
        ...
        # ── CONFIDANT → ROMANTIC_INTEREST ⭐ Main funnel fix ──
            ...
            if state.intimacy_level < 0.65:
                return False, f"Need intimacy >= 0.65, have {state.intimacy_level:.2f}", {
            ...
            if state.attachment_strength < 0.55:
                return False, f"Need attachment >= 0.55, have {state.attachment_strength:.2f}", {
            ...
            if heart_flutters < 1:
                return False, f"Need 1+ heart flutter event, have {heart_flutters}", {
            ...
            if state.current_stage == RelationshipStage.CONFIDANT.value:
                ...
                if days_in_confidant < eff_days:
                    return False, f"Need {eff_days:.0f}+ days in CONFIDANT, have {days_in_confidant:.1f}", {
        ...
        # ── ROMANTIC_INTEREST → LOVER ──
            ...
            if romantic_sessions < 2:
                return False, f"Need 2+ distinct romantic gesture sessions, have {romantic_sessions}", {
            ...
            if char_recip < 2:
                return False, f"Need 2+ character reciprocations, have {char_recip}", {
        ...
        # ── LOVER → BONDED (tuned v1.1: dual-path) ──
            ...
            if state.total_promises_made == 0:
                return False, "Need at least 1 promise made", {
            ...
            if promise_ratio < 0.8:
                return False, f"Need promise keeping ratio >= 80%, have {promise_ratio:.0%}", {
            ...
            if not ritual_path_ok and not promise_path_ok:
                return False, (
                    ...
                    "current": {"ritual_streak": ritual_streak, "shared_promises": shared_promises},
                    "required": {"ritual_streak": 21, "shared_promises": 5},
                }

            # Conflict + repair: ≥ 1
            if state.total_conflicts < 1 or state.total_successful_repairs < 1:
                return False, "Need at least 1 conflict + successful repair cycle", {
            ...
            if state.current_stage == RelationshipStage.LOVER.value:
                ...
                if days_in_lover < eff_days:
                    return False, f"Need {eff_days:.0f}+ days in LOVER, have {days_in_lover:.1f}", {
    ...
    def _check_soul_gate(
        ...
        if target_stage == RelationshipStage.ACQUAINTANCE:
            ...
            if days_since_first < required_elapsed:
                return False, f"Soul gate: need {required_elapsed:.1f} days, have {days_since_first:.1f}", {
                    "gate": "SOUL__ACQ_time",
        ...
        # ── Gate 4: ROMANTIC_INTEREST soul gate (resistance × 30) ⭐ ──
            ...
            if days_since_first < min_days:
                return False, f"Soul gate: need {min_days:.0f} days since first meeting, have {days_since_first:.0f}", {
    ...
    def _check_minimum_time(
        ...
        if days_in_stage < eff_min_days:
            return False, f"Need {eff_min_days:.1f} days in stage, have {days_in_stage:.1f}", {
                "gate": "MIN_TIME",
    ...
    def _check_anti_gaming(
        ...
        if cooldown_violations > 3:
            return False, f"Too many rapid repeated signals: {cooldown_violations}", {
    ...
    def is_empty_message(text: str) -> bool:
        ...
        emotional_keywords = [
            "想", "爱", "怕", "担心", "开心", "难过", "生气", "感动",
            "寂寞", "孤单", "喜欢", "讨厌", "希望", "害怕", "谢谢", "对不起",
            "❤", "😊", "😢", "🥺", "💕",
        ]
