...
async def start_run(
    ...
    Adult scenarios enforce age verification server-side (defense in depth on
    top of the WS/list soft-lock).
    """
    scenario = await repo.get_scenario(db, body.scenario_id)
    if scenario is None or scenario.status != "published":
        raise HTTPException(404, "scenario_not_found")

    if scenario.maturity == "adult" and not await _is_age_verified(db, current_user.user_id):
        raise HTTPException(403, "age_verification_required")
