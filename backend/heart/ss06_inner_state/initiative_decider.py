...
class SoulSpec:
    ...
    expected_gap_days: Dict[str, float] = field(
        default_factory=lambda: {
            "LOVER": 4.0,
            "BONDED": 2.0,
        }
...
class InnerStateSlice:
    ...
    anniversary_fired_log: Dict[str, int] = field(
        default_factory=dict
    )  # anniversary_id → year fired
...
def gate_safety_allows(ctx: InnerLoopContext, trigger_type: TriggerType) -> GateResult:
...
def trigger_anniversary_due(ctx: InnerLoopContext) -> TriggerResult:
    ...
    for anniv in inner.upcoming_anniversaries:
        ...
        anniv.get("soft_mention_sent", False)
...
def apply_adaptive_rate(inner: InnerStateSlice, trigger_type: TriggerType) -> Tuple[bool, str]:
...
def apply_rin_hard_cap(ctx: InnerLoopContext, trigger_type: TriggerType) -> Tuple[bool, str]:
...
class InitiativeDecider:
    ...
    def evaluate(self, ctx: InnerLoopContext) -> InitiativeDecision:
        ...
        for _trigger_type, trigger_fn in TRIGGER_EVALUATORS:
