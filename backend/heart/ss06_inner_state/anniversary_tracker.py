...
class AnniversaryTracker:
    ...
    def _resolve_due_at(
        self, src: L4AnniversarySource, now: datetime
    ) -> Optional[datetime]:
    ...
    def _extract_original_date(src: L4AnniversarySource) -> Optional[datetime]:
        ...
        date_keys = [
            "date", "datetime", "birthday", "original_date",
            "canonical_date", "occurred_at", "first_at",
        ]
    ...
    def _build_display_name(
        src: L4AnniversarySource, category: AnniversaryCategory
    ) -> str:
