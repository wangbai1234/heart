...
class InnerStateBlockBuilder:
    ...
    def _generate_concerns_section(self, inner_state: InnerState) -> str:
        ...
        for c in concerns:
            ...
            if not has_been_addressed:
                ...
                try:
                    ref_dt = datetime.fromisoformat(str(last_referenced).replace("Z", "+00:00"))
        ...
        # Sort by urgency
        active_concerns.sort(key=lambda c: getattr(c, "urgency", 0.0), reverse=True)
    ...
    def _generate_anniversary_section(self, inner_state: InnerState) -> Optional[str]:
    ...
    def _render_block_text(self, block: InnerStateBlock, modality: str) -> str:
