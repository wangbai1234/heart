...
class ProactiveMessageGenerator:
    ...
    def _build_mood_hint(inner_state: InnerState) -> str:
        ...
        if inner_state.upcoming_anniversaries:
            for a in inner_state.upcoming_anniversaries[:1]:
                name = a.get("name", "重要日子") if isinstance(a, dict) else getattr(a, "name", "重要日子")
                hours = a.get("hours_until", 0) if isinstance(a, dict) else getattr(a, "hours_until", 0)
                if hours < 24:
    ...
    def _resolve_character_name(
        character_id: str, soul: Optional[SoulSpec]
    ) -> str:
