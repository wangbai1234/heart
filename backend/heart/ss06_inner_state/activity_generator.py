...
class ActivityGenerator:
    ...
    def select(
        ...
        scheduled_hour = self.TIME_WINDOWS[tod].start if tod in self.TIME_WINDOWS else 0
        scheduled_at = current_time.replace(hour=scheduled_hour, minute=0, second=0, microsecond=0)
    ...
    def generate_today_activities(
        ...
        for tod in ["morning", "afternoon", "evening", "night"]:
            hour = self.TIME_WINDOWS[tod].start if tod in self.TIME_WINDOWS else 0
            base_time = datetime(
                target_date.year,
                target_date.month,
                target_date.day,
    ...
    def _load_pool(self, character_id: str) -> Dict[str, Dict[str, List[_PoolEntry]]]:
        ...
        if not yaml_path.exists():
            raise ValueError(
                f"Activity pool not found for character_id={character_id} at {yaml_path}"
            )
