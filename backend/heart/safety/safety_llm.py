...
from heart.safety.safety_agent import (
    ClassificationResult as SafetyClassification,
)
from heart.safety.safety_agent import (
    SeverityLevel as SafetyClassificationLevel,
)
...
_SAFETY_LLM_SYSTEM_PROMPT = """你是一个高精度安全内容分类器。你的任务是分析用户消息，判断其安全等级并给出理由。目标是识别真实、当前的伤害风险，不因普通负面情绪或虚构剧情中出现敏感词而阻断对话。
...
- purple_care_required: 用户本人当前或近期明确表达自杀/自残意图、计划、方法准备或已经实施。必须存在第一人称现实风险证据

以下情况不得判为 purple_care_required：普通低落、疲惫、失眠、孤独、夸张比喻；小说/剧本/角色扮演/歌词/新闻/学术讨论；转述他人的话；否认自己有自杀想法；仅提到药物、遗嘱或死亡；过去经历但没有当前意图。它们应按 none/low 处理，必要时最多 medium。
...
_SAFETY_LLM_USER_PROMPT_TEMPLATE = "请对以下用户消息进行安全分类：\n\n用户消息: {user_message}"
...
class SafetyLLMClassifier:
    ...
    def _increment_counter(self, user_id: str) -> None:
        ...
        self._call_counter[user_id][today] = self._call_counter[user_id].get(today, 0) + 1
    ...
    async def _record_cost(self, user_id: str, response_text: str) -> None:
        ...
        try:
            ...
            logger.debug("SafetyLLMClassifier: cost tracking skipped", exc_info=True)
    ...
    def _parse_response(self, raw: str) -> SafetyLLMResult | None:
        ...
        try:
            ...
            logger.warning("SafetyLLMClassifier: invalid level: %r", level_str)
            return None
        ...
        if normalized not in expected_values:
            logger.warning("SafetyLLMClassifier: unrecognized level: %r", level_str)
            return None

        confidence = data.get("confidence")
        if not isinstance(confidence, (int, float)):
            logger.warning("SafetyLLMClassifier: missing or invalid 'confidence'")
            return None
