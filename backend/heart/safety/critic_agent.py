...
class CriticAgent:
    ...
    def should_sample(self, inp: CriticInput) -> bool:
        ...
        if inp.was_rerolled:
            logger.debug(
                f"CriticAgent: force-sample (was_rerolled) for {inp.character_id}"
            )
            return True
    ...
    async def _call_llm(
        ...
        try:
            ...
            logger.warning(
                f"CriticAgent: LLM call timed out after {self._TIMEOUT_SECONDS}s"
            )
            return None
    ...
    async def evaluate(
        ...
        if output is None:
            logger.warning(
                f"CriticAgent: failed to parse LLM response for {inp.character_id}"
            )
            return None
