...
class TemplateLoader:
    ...
    def load_all(cls) -> None:
        ...
        if not routing_path.exists():
            msg = "FATAL: _routing.yaml not found at %s; PURPLE Care Path cannot operate"
...
class CarePathEngine:
    ...
    def emit_audit(self, event: CarePathAuditEvent) -> None:
        ...
        logger.info(
            "PURPLE Care Path audit emitted: user=%s trigger=%s template=%s locale=%s/%s",
            event.user_id,
