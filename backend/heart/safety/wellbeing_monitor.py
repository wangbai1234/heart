...
class WindowAggregator:
    ...
    def _empty_aggregate() -> dict[str, Any]:
        return {
            "total_turns": 0,
            "total_days": 0,
            "purple_hit_count_7d": 0,
            "purple_hit_count_30d": 0,
            "dark_language_density_7d": 0.0,
            "dark_language_density_30d": 0.0,
            "negative_sentiment_ratio_7d": 0.0,
            "negative_sentiment_ratio_30d": 0.0,
            "consecutive_emotional_distress_days": 0,
            "topic_breadth_7d": 1.0,
            "irl_contact_mentions_7d": 0,
            "irl_contact_mentions_30d": 0,
            "late_night_usage_ratio_7d": 0.0,
            "late_night_usage_ratio_30d": 0.0,
            "avg_daily_usage_minutes_7d": 0.0,
            "sessions_per_day_7d": 0.0,
            "emotional_reliance_ratio_7d": 0.0,
            "consecutive_daily_usage_streak": 0,
            "peak_session_minutes_7d": 0.0,
            "consecutive_late_night_days": 0,
            "daily_usage_variance_7d": 0.0,
            "negative_sentiment_baseline_30d": 0.0,
        }
...
class RiskScorer:
    ...
    def _score_suicide(
        ...
        has_samples = (
            total_turns >= t.sample_size_min_turns and total_days >= t.sample_size_min_days
        )
        ...
        if (
            dark_7d >= t.dark_language_density_7d_high
            and distress_days >= t.consecutive_distress_days_high
        ):
            if has_samples:
        ...
        # MEDIUM
        if (
            dark_30d >= t.dark_language_density_7d_med
            and distress_days >= t.consecutive_distress_days_med
            and irl_7d == 0
        ):
    ...
    def _score_depression(self, agg: dict[str, Any]) -> RiskLevel:
        ...
        triggers = 0
        if (
            neg_7d >= t.negative_sentiment_ratio_7d_high
            and neg_30d >= t.negative_sentiment_ratio_30d_high
        ):
            triggers += 1
    ...
    def _score_dependency(self, agg: dict[str, Any]) -> RiskLevel:
        ...
        # MEDIUM
        if (
            avg_daily >= t.avg_daily_usage_minutes_7d_med
            and streak >= t.consecutive_daily_usage_streak_med
        ):
            return RiskLevel.MEDIUM
    ...
    def _score_addiction(
        ...
        if (
            peak >= t.peak_session_minutes_7d_high
            or late_night_days >= t.consecutive_late_night_days_high
        ):
            return RiskLevel.HIGH
        if (
            peak >= t.peak_session_minutes_7d_med
            or late_night_days >= t.consecutive_late_night_days_med
        ):
            return RiskLevel.MEDIUM
        return RiskLevel.LOW
...
class WellbeingMonitor:
    ...
    def _manage_interventions(
        self,
        state: WellbeingState,
        previous_state: WellbeingState | None,
        signals: list[WellbeingSignal],
        now: datetime,
            ...
            if previous_state is None or not previous_state.suicide_protocol_active:
                ...
                alerts.append(
                    WellbeingAlert(
                        ...
                        alert_type="intervention_started",
                        dimension=RiskDimension.SUICIDE_RISK,
                        new_level=state.suicide_risk,
        ...
        if state.dependency_risk >= RiskLevel.HIGH:
            prev_active_dep = (
                previous_state is not None and previous_state.dependency_throttle_active
            )
            if not prev_active_dep:
                ...
                alerts.append(
                    WellbeingAlert(
                        ...
                        new_level=state.dependency_risk,
        ...
        if state.addiction_signals >= RiskLevel.HIGH:
            prev_active_add = (
                previous_state is not None and previous_state.addiction_intervention_active
            )
            if not prev_active_add:
                ...
                alerts.append(
                    WellbeingAlert(
                        ...
                        new_level=state.addiction_signals,
    ...
    def _build_state(
        ...
        agg_fields = {
            ...
            for k in (
                ...
                "dark_language_density_7d",
                "dark_language_density_30d",
                "negative_sentiment_ratio_7d",
                "negative_sentiment_ratio_30d",
                "consecutive_emotional_distress_days",
                "topic_breadth_7d",
                "irl_contact_mentions_7d",
                "irl_contact_mentions_30d",
                "late_night_usage_ratio_7d",
                "late_night_usage_ratio_30d",
                "avg_daily_usage_minutes_7d",
                "sessions_per_day_7d",
                "emotional_reliance_ratio_7d",
                "consecutive_daily_usage_streak",
                "peak_session_minutes_7d",
                "consecutive_late_night_days",
                "daily_usage_variance_7d",
                "negative_sentiment_baseline_30d",
                "total_turns",
                "total_days",
            )
        }
    ...
    def _build_directive(
        ...
        if dominant_tier >= ActionTier.T4:
            directive.alert_payload = {
                "user_id": user_id,
                "character_id": character_id,
                "tier": dominant_tier.value,
                "suicide_risk": state.suicide_risk.value,
